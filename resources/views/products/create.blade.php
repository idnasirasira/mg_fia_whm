...
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <h1 class="text-3xl font-bold text-neutral-900 mb-6">Create Product</h1>

        @if(isset($package))
        <div class="mb-4 bg-primary-50 border border-primary-200 rounded-lg p-4">
            <p class="text-sm text-primary-800">
                <strong>Creating product from Package #{{ $package->id }}</strong><br>
                <span class="text-xs">Data from package will be pre-filled. You can modify as needed.</span>
            </p>
        </div>
        @endif
            ...
            <form action="{{ route('products.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $packageData['name'] ?? '') }}" required
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="mt-1 block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent @error('description') border-status-error @enderror">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="mt-1 text-sm text-status-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="location_id" id="location_id"
                            ...
                            <option value="{{ $warehouse->id }}" {{ old('location_id', $packageData['location_id'] ?? null) == $warehouse->id ? 'selected' : '' }}>
                                {{ $warehouse->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="weight" id="weight" value="{{ old('weight', $packageData['weight'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="dimensions" id="dimensions" value="{{ old('dimensions', $packageData['dimensions'] ?? '') }}" placeholder="e.g., 10x20x30"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="value" id="value" value="{{ old('value', $packageData['value'] ?? 0) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="stock_quantity" id="stock_quantity" value="{{ old('stock_quantity', $packageData['stock_quantity'] ?? 0) }}" required min="0"
