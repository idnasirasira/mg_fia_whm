...
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <h1 class="text-3xl font-bold text-neutral-900 mb-6">Create Shipping Zone</h1>
        <div class="bg-white shadow-md rounded-xl border border-neutral-200 p-6">
            <form action="{{ route('shipping-zones.store') }}" method="POST" id="shippingZoneForm">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required
                               ...
                               placeholder="e.g., Asia Pacific, Europe, Americas">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                        <div>
                            ...
                            <input type="number" step="0.01" name="base_rate" id="base_rate" value="{{ old('base_rate', 0) }}" min="0"
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="per_kg_rate" id="per_kg_rate" value="{{ old('per_kg_rate', 0) }}" min="0"
                                   class="mt-1 block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                                   placeholder="0.00">
                        </div>
                        <div>
                            <label for="estimated_delivery" class="block text-sm font-medium text-neutral-700">Est. Delivery (days)</label>
                            <input type="number" name="estimated_delivery" id="estimated_delivery" value="{{ old('estimated_delivery') }}" min="1"
                                   class="mt-1 block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                                   placeholder="e.g., 7">
                        </div>
                    </div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 bg-primary-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700">Create Zone</button>
                </div>
            </form>
        </div>
    </div>
