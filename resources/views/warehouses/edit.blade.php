...
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        <h1 class="text-3xl font-bold text-neutral-900 mb-6">Edit Warehouse</h1>
        <div class="bg-white shadow-md rounded-xl border border-neutral-200 p-6">
            <form action="{{ route('warehouses.update', $warehouse) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="3" required
                                  class="mt-1 block w-full px-4 py-2 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 @error('address') border-status-error @enderror">{{ old('address', $warehouse->address) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="warehouse" {{ old('type', $warehouse->type) == 'warehouse' ? 'selected' : '' }}>Warehouse</option>
                                <option value="location" {{ old('type', $warehouse->type) == 'location' ? 'selected' : '' }}>Location</option>
                                <option value="zone" {{ old('type', $warehouse->type) == 'zone' ? 'selected' : '' }}>Zone</option>
                            </select>
                        </div>
                        <div>
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    ...
                    <button type="submit" class="px-4 py-2 bg-primary-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-primary-700">Update Warehouse</button>
                </div>
            </form>
        </div>
    </div>
